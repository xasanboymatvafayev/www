...
import { AppState, User, Course, Role } from './types.ts';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (savedUser && activeState.users[savedUser]) setCurrentUser(activeState.users[savedUser]);
      setIsSyncing(false);
    };
    init();
  }, []);
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            {currentView === 'tasks' && <TaskList tasks={Object.values(state.tasks)} courses={state.courses} submissions={state.submissions} currentUser={currentUser} onSubmit={s => performAction(p => ({...p, submissions: {...p.submissions, [s.id]: s}}))} onAddTask={t => performAction(p => ({...p, tasks: {...p.tasks, [t.id]: t}}))} onUpdateTask={(id, u) => performAction(p => ({...p, tasks: { ...p.tasks, [id]: { ...p.tasks[id], ...u } } }))} lang={lang} />}
